...
- Frontend implementation scope is limited to `ui/` unless the spec explicitly authorizes
  backend changes.
- The solution uses the existing React + TypeScript + Vite dashboard SPA structure rather
  than introducing a parallel frontend stack.
...
**Structure Decision**: Default to the existing monorepo layout with the Spring Boot
backend in `src/` and the SPA frontend in `ui/`. Unless the feature specification
explicitly authorizes backend changes, implementation work stays in `ui/`.
