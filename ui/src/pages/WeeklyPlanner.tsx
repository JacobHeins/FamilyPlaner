...
const sample: WeekData = {
  Monday: [{ id: 1, title: 'Football training', member: 'Luca', color: 'blue' }],
  Tuesday: [{ id: 2, title: 'Dentist', member: 'Mia', color: 'pink' }],
  Wednesday: [{ id: 3, title: 'Parent meeting', member: 'Mom', color: 'orange' }],
...
export default function WeeklyPlanner() {
  ...
  const [form, setForm] = useState({ title: '', member: MEMBERS[0], color: COLORS[0] })
  ...
  function submitAdd() {
    ...
    setData(prev => ({ ...prev, [adding]: [...(prev[adding] || []), entry] }))
    setAdding(null)
  }

  function removeEntry(day: string, id: number) {
    setData(prev => ({ ...prev, [day]: prev[day].filter(e => e.id !== id) }))
  }
    ...
    <div className="page planner-page">
      <header className="page-header">
        <div>
          ...
          <p className="page-subtitle">Plan and organise your family's upcoming week.</p>
        </div>
          ...
          return (
            <div key={day} className={`planner-col${isToday ? ' today' : ''}`}>
              <div className="planner-col-header">
                ...
                <span className={`planner-day-num${isToday ? ' today-num' : ''}`}>{dateNum}</span>
              </div>
              ...
              {adding === day ? (
                <div className="add-form">
                  ...
                  <div className="add-actions">
                    <button className="add-cancel" onClick={() => setAdding(null)}>Cancel</button>
                    <button className="add-submit" onClick={submitAdd}>Add</button>
                  </div>
                </div>
              ) : (
                <button className="planner-add-btn" onClick={() => openAdd(day)}>
                  <Plus size={14} /> Add
                </button>
              )}
            </div>
